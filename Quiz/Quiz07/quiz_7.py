...
def longest_path(i, j, grid):
    ...
    # Try both NE (↗) and SW (↙) starting directions
    path_ne = dfs(i - 1, j - 1, grid, True, path, visited)
    path_sw = dfs(i - 1, j - 1, grid, False, path, visited)
    longest = path_ne if len(path_ne) > len(path_sw) else path_sw
